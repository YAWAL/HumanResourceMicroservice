...
func InitRouter(er repository.EmployeeRepository) (r *mux.Router) {
	...
	api.HandleFunc("/employees/id/{id:[0-9a-zA-Z]+}", handlers.DeleteEmployee(er)).Methods(http.MethodDelete)
	api.HandleFunc("/employees/position/{position:[a-zA-Z]+}", handlers.ShowAllEmployeesByPosition(er)).Methods(http.MethodGet)
	return r
}
